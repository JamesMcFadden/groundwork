"""Running the golden set's questions through the service's own dense search.
...
Each question's best chunk score is recorded too, answerable and unanswerable alike: it
is the data a relevance-score threshold would be chosen from. None is applied in M3.
"""
...
def run_retrieval(
    ...
    for question in golden.answerable:
        ...
        top_5 = _search(sessions, collection_id, vector, RECALL_K)
        top_10 = _search(sessions, collection_id, vector, MRR_K)
        answerable.append(
            AnswerableResult(
                question_id=question.id,
                rank_at_5=first_hit_rank(top_5, question.evidence),
                rank_at_10=first_hit_rank(top_10, question.evidence),
                best_score=_best_score(top_5),
            )
        )
    ...
    unanswerable = [
        UnanswerableResult(
            ...
            best_score=_best_score(
                _search(sessions, collection_id, embed_query(question.question), RECALL_K)
            ),
        )
        for question in golden.unanswerable
    ]
    return RetrievalReport(answerable=tuple(answerable), unanswerable=tuple(unanswerable))


def _search(
    sessions: sessionmaker[Session], collection_id: uuid.UUID, vector: list[float], limit: int
) -> list[RetrievedChunk]:
    ...
    with sessions() as session:
        return dense_search(session, collection_id, vector, limit=limit)


def _best_score(chunks: list[RetrievedChunk]) -> float | None:
    """The score of the nearest chunk the service would answer from."""
